 ...
 *
 * Creates the following in the `genuivo` schema (all operations are idempotent):
 * 1. The `genuivo` schema itself (`CREATE SCHEMA IF NOT EXISTS`)
 * 2. The `genuivo.users` table with columns: `firebase_uid` (PK), `email`,
 *    `display_name`, `created_at`, `updated_at`
 * 3. The `genuivo.histories` table with columns: `id` (UUID PK), `user_id` (FK to users),
 *    `datetime`, `value` (numeric 12,2), `created_at`, `updated_at`
 * 4. An index on `genuivo.histories(user_id)` for efficient user-scoped queries
 *
...
export async function initDatabase() {
  ...
  await client`CREATE SCHEMA IF NOT EXISTS genuivo`;

  await client`
    CREATE TABLE IF NOT EXISTS genuivo.users (
  ...
  await client`
    CREATE TABLE IF NOT EXISTS genuivo.histories (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      user_id VARCHAR(128) NOT NULL REFERENCES genuivo.users(firebase_uid) ON DELETE CASCADE,
  ...
  await client`
    CREATE INDEX IF NOT EXISTS genuivo_histories_user_idx
    ON genuivo.histories(user_id)
  `;
